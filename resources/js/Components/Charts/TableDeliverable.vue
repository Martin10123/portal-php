<template>
    <div class="relative overflow-hidden sm:rounded-lg p-2">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody v-if="listaDataDeliverables.length">
                ...
                <tr class="border-b border-gray-200 dark:border-gray-700" v-for="deliverible in paginatedData"
                    ...
                    <th scope="row"
                        class="px-6 py-4 font-medium text-gray-900 bg-gray-50 dark:text-white dark:bg-gray-800 break-words whitespace-normal max-w-[900px]">
                        {{ deliverible.Entregable }}
                    </th>
                    <td class="px-6 py-4">{{ redondear(deliverible.total) }}</td>
                </tr>
            </tbody>
        </table>
    </div>

    <Paginator :rows="rowsPerPage" :totalRecords="listaDataDeliverables.length" :first="currentPage * rowsPerPage"
...
const props = defineProps({
    listaDataDeliverables: Array,
    totalDataDeliverables: Number,
});
